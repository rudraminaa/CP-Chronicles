...
// submission link:-https://cses.fi/problemset/result/15786959/
void solve(){
...
//-------------INPUT-------------
    ...
    for(ll i=0;i<n;i++)
    {
        while(!small.empty() && v[small.top()]>=v[i])
        {
            // jab tak koi aisa element naa mile jo v[i] se chota ho
            small.pop();
        }
        if(small.empty())
        {
            // agar koi element hai hi nhi stack mai jo v[i] se chota ho
            cout << 0 <<" "; 
        }
        else
        {
            //final output the index stored
            cout << small.top() +1 << " ";
        }
        small.push(i);
    }
