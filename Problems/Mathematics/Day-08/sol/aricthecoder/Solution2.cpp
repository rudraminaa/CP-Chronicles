...
Problem : Expected Median 1999F

Approach : A subsequence has median = 1 if it contains more than k/2 '1's
        We need to find Number of subsequences of length k that contain at least (k/2+1) '1's
        Iterate over all number of 1s in a subsequence (k/2 + 1) to min(cnt, k)
        C(cnt,i) × C(n-cnt, k-i)
        No of ones    No of Zeros
        To combinations efficiently: Precompute factorials

Time Complexity : O(n+k)
Space Somplexity : O(n)

Submission Link : https://codeforces.com/contest/1999/submission/356160165
*/
#include <bits/stdc++.h>
#include <bits/stdc++.h>
...
int C(int n, int k){
   if(n<k) return 0LL;
   return (fac[n] * binpow( (fac[n-k] * fac[k]) % MOD, MOD-2)) %MOD;
}
